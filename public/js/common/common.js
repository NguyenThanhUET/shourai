...
jQuery.fn.extend({
    _addError: function(value) {
        ...
        var message      = "";
        var messageArr   = [];
        var location    = document.getElementsByTagName('html')[0].getAttribute('lang');
        //Get message error
        if($.isArray(value)){
            $.each(value,function(key,valueItem){
                message_code = valueItem;
                if(typeof _messageTranslation !== 'undefined' && _messageTranslation[location] !== undefined && _messageTranslation[location].hasOwnProperty(message_code)){
                    messageArr.push(_messageTranslation[location][message_code]);
                }else{
            ...
            if(typeof _messageTranslation !== 'undefined'  &&  _messageTranslation[location] !== undefined && _messageTranslation[location].hasOwnProperty(message_code)){
                message =_messageTranslation[location][message_code];
            }else{
        ...
        return this.each(function() {
            //Style error
            if ($(this).next().is('span.select2.select2-container')) {
                // for select2 element
                $(this).addClass('input-error'); // add class to select2 element
                $(this).next().addClass('select2-input-error');
                $(this).next().attr('data-original-title',message);
                $(this).next().tooltip({
                    template: '<div class="tooltip tooltip-error"><div class="arrow"></div><div class="tooltip-inner"></div></div>'
                });
            }
            else {
...
function _commonShowError(object,areaSelector) {
    ...
    if (areaSelector) {
        ...
        $.each(object, function (key, value) {
            if ($('[name="' + key + '"]').hasClass('tinymce')) {
                $('#area_' + key)._addError(value);
            } else if ($('[name="' + key + '"]').hasClass('file')) {
                $('[name="' + key + '"]').parent()._addError(value);
            } else {
...
$(document).on("change dp.change",".input-error",function () {
    var key = $(this).val();
    if(key.length>0){
        $(this).removeAttr("data-original-title").removeClass("input-error");
    }
});

function _showAjaxLoading(selector) {
    if($(selector).length >0){
        if ($(selector).find("#loader").length){
...
function _hideAjaxLoading(selector){
    if($(selector).length >0){
